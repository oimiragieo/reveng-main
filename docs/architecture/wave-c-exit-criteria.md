...
| R-HEX-1 | Bounded timed `reveng analyze` on hexyl **subject** ELF; status must match probe JSON (`completed` / `timeout` / `could_not_measure`) | blocked or measured per `latest.json` — never hollow `done` |
