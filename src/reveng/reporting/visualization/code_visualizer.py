...
try:
    ...
    logger.warning("networkx unavailable (%s) - graph visualizations disabled", exc)
    nx = None  # type: ignore
...
class CallGraphBuilder:
    ...
    def _parse_java_file(self, java_file: Path):
        ...
        for match in re.finditer(method_pattern, content):
            ...
            full_name = (
                f"{package}.{class_name}.{method_name}"
                if package
...
class GraphVisualizer:
    ...
    def render_call_graph(self, graph: nx.DiGraph, output_name: str = "call_graph"):
        ...
        logger.info(
            f"Rendering call graph with {len(graph.nodes)} nodes and {len(graph.edges)} edges"
        )
...
def main():
    ...
    parser.add_argument(
        "--type",
        choices=["call-graph", "dependency", "both"],
        default="both",
        help="Visualization type",
    )
    parser.add_argument(
        "--format",
        choices=["java", "ghidra", "auto"],
        default="auto",
