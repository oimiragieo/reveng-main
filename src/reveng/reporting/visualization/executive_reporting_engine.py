...
class ExecutiveReportingEngine:
    ...
    def __init__(self, output_dir: str = "reports/executive"):
        ...
        self.risk_weights = {"likelihood": 0.4, "impact": 0.6}

        logger.info(
            f"Executive reporting engine initialized with output directory: {self.output_dir}"
        )
    ...
    def create_risk_distribution_chart(
        ...
        if not save_path:
            save_path = str(
                self.output_dir
                / f"risk_distribution_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
    ...
    def generate_executive_dashboard(
        self, analysis_results: Dict[str, Any], executive_summary: ExecutiveSummary
    ) -> str:
        """
    ...
    def generate_pdf_report(
        ...
        story.append(
            Paragraph(
                f"Assessment Date: {executive_summary.assessment_date.strftime('%Y-%m-%d')}",
                styles["Normal"],
            )
        )
        story.append(
    ...
    def _extract_risk_metrics(
        ...
        if "vulnerabilities" in analysis_results:
            ...
            for vuln in vulns.get("memory_vulnerabilities", []):
                ...
                risk_level, impact, likelihood, impact_score = severity_map.get(
            ...
            "memory_vulnerabilities": [
                {
                    ...
                    "severity": "Critical",
                    "description": "Stack buffer overflow in login function",
                    "location": "auth.c:156",
                },
                {
                    "type": "Use After Free",
                    "severity": "High",
                    "description": "Memory use after free in cleanup routine",
                    "location": "cleanup.c:89",
                },
            ]
        },
        "threat_intelligence": {
