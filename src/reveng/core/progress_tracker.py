...
class ProgressTracker:
            ...
            if analysis_id in self.analyses:
                ...
                if error_message:
                    self.analyses[analysis_id]["errors"].append(
                        {"message": error_message, "timestamp": end_time}
                    )
    ...
    def start_stage(
        self, analysis_id: str, stage_name: str, total_steps: Optional[int] = None
    ) -> None:
        ...
        with self._lock:
            if analysis_id in self.analyses:
                execution = ToolExecution(
                    tool_name=tool_name, command=command, start_time=time.time()
                )
