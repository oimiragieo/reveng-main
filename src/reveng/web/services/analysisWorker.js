...
async function processAnalysisJob(jobData) {
  ...
  try {
    ...
    if (options.enhancedAnalysis) {
      enhancedResult = await runEnhancedAnalysis(
        filePath,
        revengResult,
        options
      );
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on('close', (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(`REVENG analysis failed with code ${code}: ${stderr}`)
        );
      }
    });
  ...
  try {
    ...
    if (options.corporateExposure) {
      const response = await axios.post(
        `${config.aiServiceUrl}/analyze/corporate-exposure`,
        {
    ...
    if (options.vulnerabilityAnalysis) {
      const response = await axios.post(
        `${config.aiServiceUrl}/analyze/vulnerabilities`,
        {
          code: revengResult.decompiled_code || '',
          vuln_type: 'memory',
        },
    ...
    if (options.threatIntelligence && revengResult.indicators) {
      const response = await axios.post(
        `${config.aiServiceUrl}/analyze/threat-intelligence`,
        {
    ...
    if (options.generateDemo) {
      const response = await axios.post(
        `${config.aiServiceUrl}/generate/demonstration`,
        {
...
async function start() {
  try {
    ...
    try {
      ...
      console.warn(
        'AI service not available, enhanced analysis will be disabled'
      );
    }
    ...
    app.listen(config.port, () => {
      console.log(
        `Analysis worker health endpoint listening on port ${config.port}`
      );
    });
