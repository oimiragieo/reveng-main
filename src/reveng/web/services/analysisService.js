...
class AnalysisService {
  constructor() {
    ...
    this.config = {
      ...
      analyzerPath:
        process.env.ANALYZER_PATH || '../tools/ai_enhanced_analyzer.py',
  ...
  async createAnalysisJob(jobData) {
    ...
    console.log(
      `Analysis job created: ${analysisId} for user ${jobData.userId}`
    );
    return analysis;
  }
      ...
      analysisProcess.on('close', async (code) => {
        try {
          if (code === 0) {
            ...
            await this._handleAnalysisError(
              analysisId,
              stderr || 'Analysis process failed'
            );
          }
        } catch (error) {
  ...
  async _handleAnalysisSuccess(analysisId, stdout) {
    ...
    try {
      ...
      const resultsFile = path.join(
        analysisFolder,
        'universal_analysis_result.json'
      );
      let results = null;
  ...
  async _handleAnalysisError(analysisId, errorMessage) {
    const analysis =
      ...
      this.analysisHistory.get(analysisId);
    if (!analysis) return;
