...
class WebSocketService {
  ...
  authenticateSocket(socket, token) {
    try {
      ...
      console.log(
        `Socket authenticated: ${socket.id} for user ${decoded.username}`
      );
    } catch (error) {
  ...
  joinAnalysisRoom(socket, analysisId) {
    ...
    console.log(
      `User ${userInfo.username} joined analysis room: ${analysisId}`
    );
  }
  ...
  handleCollaborationEvent(socket, data) {
    ...
    this.io
      ...
      .emit('collaboration-event', collaborationEvent);

    console.log(
      `Collaboration event: ${eventType} in analysis ${analysisId} by ${userInfo.username}`
    );
  }
