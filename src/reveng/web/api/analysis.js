...
const fileFilter = (req, file, cb) => {
  // Allow only specific binary file types for security
  const allowedTypes = [
...
router.get('/:id/export/:format', authMiddleware, async (req, res) => {
  try {
    ...
    const exportData = await analysisService.exportResults(
      analysisId,
      format,
      req.user.id
    );
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="analysis-${analysisId}.${extensions[format]}"`
    );

    // SECURITY: Sanitize output to prevent XSS
    if (format === 'pdf') {
      // For PDF, ensure data is properly encoded
      res.send(Buffer.from(exportData, 'base64'));
    } else {
      // For JSON, use proper JSON response method
      res.json(exportData);
    }
  } catch (error) {
