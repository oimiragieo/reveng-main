#!/usr/bin/env python3
"""
REVENG Anthropic Claude Integration
=====================================

Integrates Anthropic Claude for AI-powered code analysis with:
- Lazy import (does not fail if anthropic is not installed)
...
Supported models:
  - claude-opus-4-6        (primary — 1M context)
...
_INSTALL_HINT = (
    "The 'anthropic' package is required for AnthropicAnalyzer. "
    "Install it with: pip install reveng[ai]"
)


def _import_anthropic():
    """Lazy-import anthropic and raise a clear error when absent."""
    try:
...
def _extract_code_block(text: str) -> str:
    ...
    if "```" not in text:
        return text.strip()
    # Split on ``` boundaries; odd-indexed parts are inside fences
    parts = text.split("```")
    for i in range(1, len(parts), 2):
        content = parts[i]
        # Strip the optional language identifier line (e.g. "c\n", "python\n")
...
class AnthropicAnalyzer:
    """
    Anthropic Claude-powered code analysis.
    ...
    def __init__(
        ...
        """
        Initialize AnthropicAnalyzer.

        Args:
            api_key: Anthropic API key.  Defaults to ``ANTHROPIC_API_KEY`` env var.
            model_id: Model to use (default: ``claude-opus-4-6``).
        ...
        logger.info(
            "AnthropicAnalyzer ready: model=%s max_tokens=%d temperature=%.2f timeout=%ds",
            self.model_id,
            self.max_tokens,
            self.temperature,
    ...
    def analyze_function(
        ...
        """
        Analyze a C function using Claude.
        ...
        Returns:
            AnalysisResult populated from Claude's response.
        """
        ...
        try:
            ...
            logger.error("Anthropic analysis failed for %s: %s", function_name, exc)
    ...
    def generate_implementation(self, function_spec: Dict[str, Any], language: str = "c") -> str:
        ...
        try:
            ...
            logger.error("Anthropic code generation failed: %s", exc)
    ...
    def _call_anthropic(
        ...
        """
        Send a message to the Anthropic Messages API with exponential backoff.
            ...
            try:
                ...
                for block in message.content:
                    if hasattr(block, "text"):
                        ...
                        logger.debug(
                            "Anthropic call succeeded on attempt %d/%d "
                            "(tokens_used=%d, model=%s)",
                            attempt + 1,
                            max_retries,
                            tokens_used,
                            self.model_id,
                ...
                logger.warning(
                    "Anthropic rate-limit hit (attempt %d/%d). Sleeping %ds.",
                    attempt + 1,
                    max_retries,
                    delay,
                )
                ...
                delay = backoff_delays[min(attempt, len(backoff_delays) - 1)]
                logger.warning(
                    "Anthropic connection error (attempt %d/%d): %s. Sleeping %ds.",
                    attempt + 1,
                    max_retries,
                    exc,
                    delay,
    ...
    def _fallback_analysis(self, function_name: str, function_code: str) -> AnalysisResult:
        ...
        return AnalysisResult(
            ...
            suggested_name=None,
            confidence=0.2,
            reasoning="Fallback heuristic analysis (Anthropic API unavailable)",
        )
