...
class EnhancedAIAnalyzer:
    ...
    def analyze_functions(
        self, functions: List[Dict[str, str]], output_folder: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Analyzing {total} functions with {'AI' if self.ai_analyzer else 'heuristics'}"
        )
            ...
            if self.ai_analyzer:
                ...
                try:
                    result = self.ai_analyzer.analyze_function(
                        func["code"], func_name, func.get("context")
                    )
    ...
    def _generate_report(self, results: Dict, total: int) -> Dict[str, Any]:
        ...
        for func_name, analysis in results.items():
            ...
            for concern in concerns:
                security_issues.append({"function": func_name, "concern": concern})
    ...
    def _save_results(self, results: Dict, report: Dict, output_folder: Path):
    ...
    def _save_markdown_report(self, report: Dict, results: Dict, output_file: Path):
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            if report["high_confidence_functions"]:
                f.write(
                    f"## High Confidence Functions ({len(report['high_confidence_functions'])})\n\n"
                )
