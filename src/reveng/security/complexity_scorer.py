...
class ComplexityScorer:
    ...
    def analyze_directory(
        self, directory: Path, pattern: str = "**/*.c"
    ...
    def _analyze_function(
        self, func_name: str, file_path: str, func_body: str
    ) -> ComplexityMetrics:
        """Analyze a single function"""
        metrics = ComplexityMetrics(function_name=func_name, file_path=file_path)
    ...
    def _count_branches(self, func_body: str) -> int:
        ...
        branch_keywords = [r"\bif\b", r"\belse\b", r"\bswitch\b", r"\bcase\b"]
    ...
    def _calculate_score(self, metrics: ComplexityMetrics) -> float:
        ...
        score = sum(normalized[name] * weights[name] for name in weights.keys())
    ...
    def _analyze_issues(self, metrics: ComplexityMetrics):
        ...
        if metrics.lines_of_code > self.thresholds["loc"]["high"]:
            metrics.issues.append(f"Very long function ({metrics.lines_of_code} LOC)")
            metrics.recommendations.append("Split into multiple smaller functions")
        elif metrics.lines_of_code > self.thresholds["loc"]["medium"]:
            metrics.issues.append(f"Long function ({metrics.lines_of_code} LOC)")
            metrics.recommendations.append("Consider extracting helper functions")

        # Nesting depth
        if metrics.nesting_depth > self.thresholds["nesting"]["high"]:
            metrics.issues.append(f"Very deep nesting ({metrics.nesting_depth} levels)")
            metrics.recommendations.append("Use early returns or extract nested logic")
        elif metrics.nesting_depth > self.thresholds["nesting"]["medium"]:
            metrics.issues.append(f"Deep nesting ({metrics.nesting_depth} levels)")
            metrics.recommendations.append("Consider flattening nested conditionals")

        # Branch count
        if metrics.branch_count > self.thresholds["branches"]["high"]:
            metrics.issues.append(f"Too many branches ({metrics.branch_count})")
            metrics.recommendations.append("Use lookup tables or strategy pattern")

    def generate_report(
        self, metrics_list: List[ComplexityMetrics], output_path: Optional[Path] = None
    ) -> Dict:
        """
        ...
        level_counts = {"low": 0, "medium": 0, "high": 0, "very_high": 0}
    ...
    def print_summary(self, report: Dict):
        ...
        print(
            f"  Low:       {dist['low']:4d} ({dist['low'] / summary['total_functions'] * 100:.1f}%)"
        )
        print(
            f"  Medium:    {dist['medium']:4d} ({dist['medium'] / summary['total_functions'] * 100:.1f}%)"
        )
        print(
            f"  High:      {dist['high']:4d} ({dist['high'] / summary['total_functions'] * 100:.1f}%)"
        )
        print(
            f"  Very High: {dist['very_high']:4d} ({dist['very_high'] / summary['total_functions'] * 100:.1f}%)"
        )
        print()
        ...
        for i, func in enumerate(report["top_10_complex"], 1):
            ...
            print(
                f"   Metrics: CC={func['metrics']['cyclomatic']}, "
