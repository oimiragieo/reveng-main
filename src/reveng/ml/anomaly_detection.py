...
class MLAnomalyDetection:
    ...
    def _extract_api_features(self, analysis_data: Dict[str, Any]) -> List[AnomalyFeature]:
        ...
        try:
            ...
            if "api_analysis" in analysis_data:
                ...
                # API diversity
                api_diversity = len(
                    set(api.get("api", "") for api in api_analysis.get("api_calls", []))
                )
                features.append(
                    AnomalyFeature(
                        name="api_diversity",
    ...
    def _extract_resource_features(self, analysis_data: Dict[str, Any]) -> List[AnomalyFeature]:
        ...
        try:
            ...
            if "pe_resources" in analysis_data:
                ...
                # Resource count
                resource_count = (
                    len(pe_resources.get("icons", []))
                    + len(pe_resources.get("strings", []))
                    + len(pe_resources.get("manifests", []))
                )
                features.append(
                    AnomalyFeature(
                        name="resource_count",
    ...
    def _extract_file_operation_features(
    ...
    def _detect_behavioral_anomalies(
        self, features: List[AnomalyFeature], analysis_data: Dict[str, Any]
    ) -> List[AnomalyResult]:
        ...
        try:
            ...
            suspicious_api_count = next(
                (f.value for f in features if f.name == "suspicious_api_count"), 0
            )
    ...
    def _detect_pattern_anomalies(
        ...
        try:
            ...
            suspicious_string_count = next(
                (f.value for f in features if f.name == "suspicious_string_count"), 0
            )
