...
try:
    ...
    try:
        ...
        from reveng.tools.utils.reconstruction_comparator import (
            ComparisonResult,
...
class CompilerConfig:
    ...
    def detect_toolchain(cls, target_arch: Architecture) -> Optional["CompilerConfig"]:
        ...
        logger.info(
            f"Detecting toolchain: system={system}, machine={machine}, target={target_arch.name}"
        )
        ...
        try:
            ...
            if self.documentation_config.generate_build_scripts:
                build_scripts.extend(
                    self._generate_build_scripts(c_source_dir, output_path, warnings)
                )
            ...
            if (
                self.documentation_config.enable_comparison
                ...
                try:
                    ...
                    comparison_result = comparator.compare_binaries(
                        self.original_binary, executable
                    )
                    ...
                    logger.info(
                        f"Comparison completed. Accuracy: {comparison_result.metrics.overall_accuracy:.2%}"
                    )
            ...
            logger.info(
                f"Original size: {size_original} bytes, Reassembled: {size_reassembled} bytes"
            )
 ...
 */"""
    ...
    def _generate_build_scripts(
        self, source_dir: Path, output_path: Path, warnings: List[str]
    ) -> List[Path]:
...
"""
    ...
    def _create_failure_result(
        self, errors: List[str], warnings: List[str], time: float
    ) -> ReassemblyResult:
...
def main():
    ...
    parser.add_argument(
        ...
        default="detailed",
        help="Documentation comment style",
    )
    parser.add_argument(
        "--no-build-scripts",
        action="store_true",
        help="Disable build script generation",
    )
    parser.add_argument(
        "--no-comparison", action="store_true", help="Disable reconstruction comparison"
    )
    ...
    if args.validation_config:
        if BinaryValidator:
            ...
            logger.warning("BinaryValidator not available, skipping validation config")
            val_config = None
    else:
