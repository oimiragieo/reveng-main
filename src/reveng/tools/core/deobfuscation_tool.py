...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("deobfuscation_tool.log"), logging.StreamHandler()],
)
...
class DeobfuscationTool:
    ...
    def deobfuscate_binary(self, binary_path: str):
        """
        Deobfuscate binary and split into domains (wrapper for deobfuscate_application)

        This method provides compatibility with the analyzer pipeline.
        It accepts a binary_path parameter and delegates to deobfuscate_application().

        Args:
            binary_path: Path to the binary file to deobfuscate

        Returns:
            Dictionary containing deobfuscated domain structure
        """
        logger.info(f"Deobfuscating binary: {binary_path}")
        # Store binary path for potential future use
        self.binary_path = Path(binary_path)
        # Delegate to existing deobfuscation logic
        return self.deobfuscate_application()
        ...
        try:
            ...
            if domain:
                self.domain_categories[domain]["functions"].append(
                    {"name": func_name, "file": c_file, "domain": domain}
                )
                ...
                self.domain_categories["utility"]["functions"].append(
                    {"name": func_name, "file": c_file, "domain": "utility"}
                )
...
"""
    ...
    def _generate_domain_documentation(self):
        ...
        main_doc = """# Deobfuscated Application Documentation
...
def main():
    ...
    print("  - Output Folder: deobfuscated_app/")
    print()
    ...
    for domain, config in tool.domain_categories.items():
        if config["functions"]:
            ...
            print("    - [function].c (individual functions)")
    print()
