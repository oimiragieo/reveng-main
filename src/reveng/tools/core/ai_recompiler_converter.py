...
class AIRecompilerConverter:
    ...
    def _generate_ai_summary(
        self, func_data: Dict[str, Any], evidence: List[Evidence]
    ) -> FunctionSummary:
    ...
    def _verify_ai_claims(
        self, summary: FunctionSummary, evidence: List[Evidence]
    ) -> FunctionSummary:
    ...
    def generate_prototype_suggestions(
        self, func_data: Dict[str, Any]
    ) -> List[PrototypeSuggestion]:
    ...
    def generate_analysis_report(self):
        ...
        report = {
            ...
            "function_summaries": [
                self._serialize_function_summary(fs) for fs in self.function_summaries
            ],
            "rename_suggestions": [
            ...
            "statistics": {
                ...
                "high_confidence_suggestions": len(
                    [rs for rs in self.rename_suggestions if rs.confidence > 0.8]
                ),
                "clusters_identified": len(self.clusters),
                "iocs_found": len(self.ioc_hints),
                "average_confidence": (
                    sum(fs.confidence for fs in self.function_summaries)
                    / len(self.function_summaries)
                    if self.function_summaries
                    else 0
                ),
            },
        }
...
def main():
    ...
    print(
        f"  - High Confidence Suggestions: {results['statistics']['high_confidence_suggestions']}"
    )
