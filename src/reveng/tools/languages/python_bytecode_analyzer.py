...
class PythonBytecodeDetector:
    ...
    @staticmethod
    def _is_trusted_source(file_path: Path) -> bool:
        """Check if the source is trusted for marshal deserialization"""
        # SECURITY: Only allow marshal loading from trusted sources
        trusted_paths = [
            "/usr/lib/python",
        ...
        return any(trusted_path in file_path_str for trusted_path in trusted_paths)
            ...
            if hasattr(code, "co_consts"):
                for const in code.co_consts:
                    if hasattr(const, "co_code"):
                        nested_imports, nested_funcs, nested_classes = (
                            PythonBytecodeDetector._analyze_code_object(const)
                        )
...
class PythonDecompiler:
    ...
    def decompile(
        self, pyc_file: str, output_file: str, python_version: str
    ) -> DecompilationResult:
...
class PythonBytecodeAnalyzer:
    ...
    def _generate_report(
        self, info: PythonBytecodeInfo, result: DecompilationResult, output_dir: Path
    ):
