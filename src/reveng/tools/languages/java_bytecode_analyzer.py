...
class JavaBytecodeAnalyzer:
    ...
    def _run_decompiler(
        self, decompiler: str, class_file: Path, config: Dict
    ) -> DecompilationResult:
        ...
        try:
            ...
            logger.debug(f"Executing: {' '.join(command_parts)}")

            # Run decompiler with timeout (SECURITY: shell=False to prevent injection)
            result = subprocess.run(
    ...
    def _detect_obfuscation(
        ...
        decompilation_results: List[DecompilationResult],
    ) -> bool:
        """
