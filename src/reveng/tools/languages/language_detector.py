...
class LanguageDetector:
    ...
    def detect(self, file_path: str) -> FileTypeInfo:
        ...
        result = (
            self._detect_by_magic_bytes(file_path)
            or self._detect_by_extension(file_path)
            or self._detect_by_structure(file_path)
            or self._detect_javascript_nodejs(file_path)
            or self._detect_electron_app(file_path)
            or FileTypeInfo(
                language="unknown",
                format="unknown",
                confidence=0.0,
        ...
        logger.info(
            f"Detected {file_path.name} as {result.language}/{result.format} (confidence: {result.confidence:.2f})"
        )
        ...
        try:
            ...
            if header.startswith(b"\xca\xfe\xba\xbe"):
                return FileTypeInfo(
                    ...
                    format="class",
                    confidence=1.0,
                    details={"method": "magic_bytes", "signature": "CAFEBABE"},
                )
            ...
            if header.startswith(b"\x7fELF"):
                return FileTypeInfo(
                    ...
                    format="elf",
                    confidence=1.0,
                    details={"method": "magic_bytes", "signature": "ELF"},
                )
            ...
            if header.startswith(b"MZ"):
                ...
                if self._is_dotnet_assembly(file_path):
                    ...
                    return FileTypeInfo(
                        ...
                        format="pe",
                        confidence=1.0,
                        details={"method": "magic_bytes", "signature": "MZ"},
                    )
            ...
            if header[:4] in [b"\xfe\xed\xfa\xce", b"\xfe\xed\xfa\xcf"]:
                return FileTypeInfo(
                    ...
                    format="mach_o",
                    confidence=1.0,
                    details={"method": "magic_bytes", "signature": "FEEDFACE/FEEDFACF"},
                )
            ...
            for pyc_magic in self.MAGIC_BYTES["PYTHON_PYC"]:
                if header.startswith(pyc_magic):
                    return FileTypeInfo(
                        ...
                        format="pyc",
                        confidence=0.95,
                        details={"method": "magic_bytes", "signature": pyc_magic.hex()},
                    )
            ...
            if header_str.startswith('{"files":') or header_str.startswith('{"files" :'):
                return FileTypeInfo(
                    ...
                    confidence=0.9,
                    details={"method": "magic_bytes", "signature": "ASAR_JSON"},
                )
    ...
    def _detect_by_extension(self, file_path: Path) -> Optional[FileTypeInfo]:
        ...
        if ext in [".jar", ".war", ".ear"]:
            return FileTypeInfo(
                ...
                format=ext[1:],
                confidence=0.85,
                details={"method": "extension"},  # jar/war/ear
            )
    ...
    def _detect_javascript_nodejs(self, file_path: Path) -> Optional[FileTypeInfo]:
        ...
        try:
            ...
            if file_path.suffix.lower() == ".js":
                ...
                nodejs_matches = sum(
                    1 for pattern in nodejs_patterns if re.search(pattern, content)
                )
                ...
                if nodejs_matches >= 2:
                    ...
                    return FileTypeInfo(
                        ...
                        format="minified_js",
                        confidence=0.7,
                        details={"method": "content_analysis", "minified": True},
                    )
        ...
        try:
            ...
            if file_path.name == "app.asar":
                return FileTypeInfo(
                    ...
                    format="electron_asar",
                    confidence=0.95,
                    details={"method": "filename_analysis", "asar_file": True},
                )
    ...
    def get_analysis_priority(self, file_type_info: FileTypeInfo) -> str:
        ...
        elif file_type_info.language in ["java", "python"] or file_type_info.format in [
            "electron_app",
            "electron_asar",
        ]:
            return "medium"
