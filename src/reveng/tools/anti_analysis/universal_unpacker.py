...
class UniversalUnpacker:
    ...
    def unpack(
        ...
        if not output_path:
            ...
            output_path = str(
                packed_path.parent / f"{packed_path.stem}_unpacked{packed_path.suffix}"
            )
        ...
        if method == "auto" or method == "specialized":
            ...
            if packer_info.packer_name:
                result = self._specialized_unpack(
                    packed_binary, output_path, packer_info.packer_name
                )

                if result and result.success:
                    logger.info(
                        f"Successfully unpacked using specialized method for {packer_info.packer_name}"
                    )
                    return result
    ...
    def _specialized_unpack(
        self, packed_binary: str, output_path: str, packer_name: str
    ...
    def _unpack_upx(self, packed_binary: str, output_path: str, original_hash: str) -> UnpackResult:
        ...
        try:
            ...
            result = subprocess.run(
                ["upx", "-d", output_path], capture_output=True, text=True, timeout=30
            )
