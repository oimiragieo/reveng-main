...
class PatchAnalyzer:
    ...
    def analyze_patch(
        self, unpatched_binary: str, patched_binary: str, cve: Optional[str] = None
    ) -> List[Vulnerability]:
        """
        ...
        for mod_func in diff_result.modified_functions:
            ...
            if mod_func.similarity < 0.95:  # More than 5% changed
                vuln = self._analyze_function_changes(mod_func, diff_result, cve)
    ...
    def _analyze_function_changes(
        self, mod_func, diff_result: DiffResult, cve: Optional[str]
