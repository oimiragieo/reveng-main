...
class CompilationTester:
    ...
    def _detect_compilers(self) -> Dict[str, Optional[str]]:
        ...
        compilers = {"msvc": None, "mingw": None, "gcc": None, "clang": None}
    ...
    def test_single_file(
        ...
        compile_cmd = self._build_compile_command(
            compiler, source_file, output_file, compile_only=True
        )
    ...
    def test_directory(
        ...
        if not c_files:
            return {"status": "error", "error": f"No C files found in {source_dir}"}
    ...
    def test_link(
        ...
        compiler = compiler or self.default_compiler
        if not compiler:
            return {"status": "error", "error": "No compiler available"}

        # Build link command
        link_cmd = self._build_link_command(compiler, object_files, output_binary)
        ...
        try:
            ...
            return {"status": "timeout", "error": "Linking timed out after 60 seconds"}
        except Exception as e:
    ...
    def _build_link_command(
        self, compiler: str, object_files: List[Path], output_binary: Path
    ) -> List[str]:
