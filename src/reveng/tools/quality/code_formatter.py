...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("code_formatter.log"), logging.StreamHandler()],
)
...
class CodeFormatter:
    ...
    def format_python_code(self, file_path: Path) -> Tuple[bool, str]:
        ...
        try:
            # Run black
            result = subprocess.run(
                ["black", str(file_path)], capture_output=True, text=True, timeout=30
            )
    ...
    def format_directory(self, directory: Path, file_pattern: str = "*.c") -> Dict[str, any]:
        ...
        for file_path in files:
            ...
            if (
                file_pattern.endswith(".c")
                or file_pattern.endswith(".cpp")
                or file_pattern.endswith(".h")
            ):
        ...
        logger.info(
            f"Formatting complete: {results['formatted']} formatted, {results['failed']} failed"
        )
        logger.info(
            f"Static analysis: {results['analysis']['warnings']} warnings, {results['analysis']['errors']} errors"
        )
