...
class PluginManager:
    ...
    def _load_plugin_metadata(self, plugin_json_path: Path):
        ...
        plugin_info = PluginInfo(
            metadata=metadata, plugin_instance=None, enabled=False, loaded=False
        )
    ...
    def _check_dependencies(self, metadata: PluginMetadata) -> bool:
        ...
        for dep in metadata.dependencies:
            # SECURITY: Only allow whitelisted dependencies
            if dep not in allowed_dependencies:
                logger.error(f"Unauthorized dependency: {dep}")
                return False
            try:
    ...
    def _is_safe_module_name(self, module_name: str) -> bool:
        """Validate module name to prevent arbitrary imports"""
        ...
        # SECURITY: Prevent system module imports
        forbidden_modules = {
            "os",
            "sys",
            "subprocess",
    ...
    def create_plugin_template(
        self, name: str, plugin_type: PluginType, output_dir: Optional[Path] = None
    ):
