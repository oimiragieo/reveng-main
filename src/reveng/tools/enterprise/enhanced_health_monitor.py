...
class AIServiceHealthChecker(HealthChecker):
    ...
    def check_health(self) -> ComponentHealth:
        ...
        try:
            from ..ai.ollama_preflight import OllamaPreflightChecker
        ...
        try:
            from ..config.enhanced_config_manager import get_enhanced_config
    ...
    def get_health_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for component_name in self.checkers.keys():
            component_health_data = [
                h.components.get(component_name)
                for h in recent_health
                if h.components.get(component_name)
            ]
