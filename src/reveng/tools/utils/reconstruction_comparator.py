...
class ReconstructionComparator:
    ...
    def _check_tool(self, tool_name: str) -> bool:
        ...
        try:
            result = subprocess.run(
                [tool_name, "--version"], capture_output=True, timeout=5, check=False
            )
    ...
    def compare_binaries(self, original: Path, reconstructed: Path) -> ComparisonResult:
        ...
        try:
            ...
            result.metrics.binary_similarity = self._compare_binary_structure(
                original, reconstructed, result
            )

            # 2. Functional equivalence testing
            logger.info("Testing functional equivalence...")
            result.metrics.functional_equivalence = self._test_functional_equivalence(
                original, reconstructed, result
            )

            # 3. Behavioral analysis
            logger.info("Performing behavioral analysis...")
            result.metrics.behavioral_match = self._analyze_behavior(
                original, reconstructed, result
            )

            # 4. Performance comparison
            logger.info("Comparing performance...")
            result.metrics.performance_ratio = self._compare_performance(
                original, reconstructed, result
            )

            # 5. Security analysis
            logger.info("Analyzing security properties...")
            result.metrics.security_parity = self._analyze_security_properties(
                original, reconstructed, result
            )
            ...
            logger.info(
                f"Comparison completed. Overall accuracy: {result.metrics.overall_accuracy:.2%}"
            )
    ...
    def _compare_binary_structure(
        self, original: Path, reconstructed: Path, result: ComparisonResult
    ) -> float:
        ...
        try:
            ...
            size_ratio = (
                min(orig_size, recon_size) / max(orig_size, recon_size)
                if max(orig_size, recon_size) > 0
                else 0
            )
            ...
            if self.has_hexdump:
                ...
                result.binary_differences.append(
                    {"type": "binary_content", "similarity": content_similarity}
                )
            ...
            if self.has_strings:
                ...
                result.binary_differences.append(
                    {"type": "strings_content", "similarity": strings_similarity}
                )
            ...
            if self.has_readelf:
                ...
                result.binary_differences.append(
                    {"type": "binary_structure", "similarity": structure_similarity}
                )
    ...
    def _compare_binary_headers(self, original: Path, reconstructed: Path) -> float:
        ...
        try:
            ...
            for field_name in common_fields:
                if field_name in orig_headers and field_name in recon_headers:
                    if orig_headers[field_name] == recon_headers[field_name]:
    ...
    def _test_functional_equivalence(
        self, original: Path, reconstructed: Path, result: ComparisonResult
    ) -> float:
    ...
    def _measure_execution_time(self, binary_path: Path) -> float:
        ...
        try:
            ...
            subprocess.run([str(binary_path)], capture_output=True, timeout=5, check=False)
    ...
    def _analyze_security_properties(
        self, original: Path, reconstructed: Path, result: ComparisonResult
    ) -> float:
    ...
    def _generate_recommendations(self, result: ComparisonResult) -> List[str]:
        ...
        if result.metrics.binary_similarity < 0.7:
            recommendations.append(
                "Low binary similarity detected. Review reconstruction algorithms and improve code generation."
            )

        # Functional equivalence recommendations
        if result.metrics.functional_equivalence < 0.8:
            recommendations.append(
                "Functional differences found. Verify control flow reconstruction and data handling logic."
            )

        # Behavioral recommendations
        if result.metrics.behavioral_match < 0.7:
            recommendations.append(
                "Behavioral differences detected. Check system call patterns and library dependencies."
            )

        # Performance recommendations
        if result.metrics.performance_ratio < 0.5:
            recommendations.append(
                "Significant performance difference. Optimize generated code and compiler flags."
            )

        # Security recommendations
        if result.metrics.security_parity < 0.8:
            recommendations.append(
                "Security feature mismatch. Ensure security protections are properly reconstructed."
            )

        # Overall accuracy recommendations
        if result.metrics.accuracy_level == AccuracyLevel.POOR:
            recommendations.append(
                "Overall reconstruction quality is poor. Consider manual review and refinement."
            )
        elif result.metrics.accuracy_level == AccuracyLevel.FAIR:
            recommendations.append(
                "Reconstruction quality is fair. Focus on improving weak areas identified above."
            )
