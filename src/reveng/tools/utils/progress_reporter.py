...
class ProgressReporter:
    ...
    def step(
        ...
        return tqdm(iterable, desc=desc, total=total, unit=unit, leave=False, ncols=100)

    def track(self, total: int, desc: str, unit: str = "item") -> "ProgressBar":
        """
...
class ProgressBar:
    ...
    def __init__(self, total: int, desc: str, unit: str):
        ...
        self.pbar = tqdm(total=total, desc=desc, unit=unit, leave=False, ncols=100)
...
class PipelineProgress:
    ...
    def start_step(self, step_num: int, step_name: str, substeps: Optional[int] = None):
        ...
        if substeps:
            self.step_bar = tqdm(
                total=substeps, desc=f"  {step_name}", unit="op", leave=False, ncols=100
            )
