...
class EnhancedCodeGenerator:
    ...
    def _load_ai_analysis(self) -> Dict:
        ...
        with open(ai_report, "r", encoding="utf-8") as f:
            data = json.load(f)
            logger.info(
                f"Loaded AI analysis with {len(data.get('function_summaries', []))} functions"
            )
            return data
...
def main():
    ...
    parser.add_argument(
        "--ai-analysis",
        default="ai_recompiler_analysis_droid",
        help="AI analysis directory",
    )
    parser.add_argument(
        "--ghidra-analysis",
        default="src_optimal_analysis_droid",
        help="Ghidra analysis directory",
    )
    parser.add_argument("--output", default="enhanced_code", help="Output directory")
