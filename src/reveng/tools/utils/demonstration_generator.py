...
class DemonstrationGenerator:
    ...
    def create_demonstration(
        self, analysis_results: Dict[str, Any], config: DemoConfig
        ...
        try:
            ...
            package.recommendations = self._generate_recommendations(
                evidence, config.target_audience
            )
                ...
                if vuln_data.get("memory_vulnerabilities"):
                    evidence.append(
                        AnalysisEvidence(
                            ...
                            risk_level=RiskLevel.CRITICAL,
                            technical_details={
                                "vulnerabilities": vuln_data["memory_vulnerabilities"]
                            },
    ...
    def _generate_recommendations(
        self, evidence: List[AnalysisEvidence], audience: str
    ) -> List[str]:
    ...
    def _create_technical_analysis(
        ...
        # Note: _generate_code_samples method not yet implemented
...
<body>
    ...
    def _format_evidence_for_executives(self, evidence: List[AnalysisEvidence]) -> str:
        ...
        for item in evidence:
            ...
            formatted.append(
                f"**{risk_emoji} {item.evidence_type.replace('_', ' ').title()}**: {item.description}"
            )
        return "\n".join(formatted)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Type of demonstration to create",
    )
    parser.add_argument(
        "--audience",
