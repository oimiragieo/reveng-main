...
class MLPipelineOrchestrator:
    ...
    def run_ml_pipeline(
        self, file_path: str, reveng_results: Dict[str, Any], file_type: Any = None
    ) -> MLPipelineResult:
        """
        ...
        try:
            ...
            if self.enable_nlp_analysis and self.nlp_analyzer:
                nlp_start = time.time()
                try:
                    code_summary = self._run_nlp_analysis(reveng_results, file_type)
                    ...
                    self.logger.info(
                        f"NLP analysis completed: {len(code_summary.algorithms_used)} algorithms detected"
                    )
    ...
    def _run_vulnerability_prediction(
        self, reveng_results: Dict[str, Any], file_type: Any = None
        ...
        if code:
            ...
            self.logger.info(
                f"Predicted {len(vulnerabilities)} vulnerabilities, {len(high_confidence_vulns)} high-confidence"
            )
    ...
    def _run_malware_classification(
        self, file_path: str, reveng_results: Dict[str, Any]
    ) -> MalwareClassification:
        ...
        self.logger.info(
            f"Classified as {classification.family} with {classification.confidence:.2f} confidence"
        )
    ...
    def _run_nlp_analysis(
        ...
        if code:
            ...
            self.logger.info(
                f"Generated code summary with {len(code_summary.algorithms_used)} algorithms"
            )
            return code_summary

        # Return empty summary if no code available
        from ..ai.ai_enhanced_data_models import CodeSemantics, CodeSummary
    ...
    def create_enhanced_result(
        self,
        file_info: Any,
        reveng_results: Dict[str, Any],
        ml_pipeline_result: MLPipelineResult,
    ) -> EnhancedUniversalAnalysisResult:
        ...
        malware_classification = (
            ml_pipeline_result.malware_classifications[0]
            if ml_pipeline_result.malware_classifications
            else None
        )
        code_summary = (
    ...
    def _calculate_confidence_scores(self, ml_result: MLPipelineResult) -> Dict[str, float]:
        ...
        if ml_result.vulnerability_predictions:
            avg_vuln_confidence = sum(
                v.confidence for v in ml_result.vulnerability_predictions
            ) / len(ml_result.vulnerability_predictions)
