...
class FunctionalCodeGenerator:
        ...
        if self.use_ai:
            try:
                ...
                if ai_config.enable_ai:
                    ...
                    logger.info(
                        f"AI code generation enabled with model: {self.ai_analyzer.model_name}"
                    )
                else:
                    self.use_ai = False
    ...
    def _generate_with_ai(
        self, function_name: str, disassembly: str, analysis: Optional[Dict[str, Any]]
    ) -> str:
        ...
        try:
            ...
            # Add header comments
            full_code = self._add_header(
                function_name, "AI-generated functional implementation", analysis
            )
            full_code += "\n" + c_code
...
"""
    ...
    def _add_header(
        self, function_name: str, description: str, analysis: Optional[Dict[str, Any]]
    ) -> str:
...
if __name__ == "__main__":
    ...
    code = generator.generate_functional_code(
        args.function_name, disassembly, analysis, args.output
    )
