...
class PEAnalyzerPlugin(AnalysisPlugin):
    ...
    def analyze(self, context: PluginContext) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"PE analysis completed: {len(sections)} sections, {len(imports)} imports, {len(exports)} exports"
            )

            return {"pe_info": pe_info, "analysis_type": "pe_analysis", "success": True}

        except Exception as e:
            logger.error(f"PE analysis failed: {e}")
            return {"analysis_type": "pe_analysis", "success": False, "error": str(e)}
