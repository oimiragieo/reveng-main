...
class TestInstallation:
    ...
    def test_compiler_availability(self):
        ...
        # On Windows, compilers are optional (MSVC is the default)
        ...
        try:
            result = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "reveng.tools.binary.check_toolchain",
                    "--check-only",
                ],
                capture_output=True,
                text=True,
...
class TestFileStructure:
    ...
    def test_directories_exist(self):
        ...
        required_dirs = ["src/reveng", "tests", "examples", "docs"]
    ...
    def test_tools_directory(self):
        ...
        tools_dir = Path("src/reveng/tools")
        assert tools_dir.exists()

        # Check for key tool subdirectories
        key_subdirs = [
            "core",
            "languages",
            "binary",
        ...
        for subdir in key_subdirs:
            ...
            assert subdir_path.exists(), f"Tool subdirectory not found: {subdir}"
