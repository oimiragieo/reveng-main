...
class TestCLI:
    ...
    def test_verbose_output(self):
        ...
        result = subprocess.run(
            [sys.executable, "reveng_analyzer.py", str(java_sample), "--verbose"],
            capture_output=True,
            text=True,
            timeout=60,
        )
...
class TestBootstrapScripts:
    ...
    def test_linux_bootstrap(self):
        ...
        result = subprocess.run(
            ["bash", "-n", str(bootstrap_script)], capture_output=True, text=True, timeout=10
        )
...
class TestUtilityScripts:
    ...
    def test_lint_script(self):
        ...
        result = subprocess.run(
            [sys.executable, str(lint_script), "--help"], capture_output=True, text=True, timeout=30
        )
