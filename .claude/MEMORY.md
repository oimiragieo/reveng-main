...
- Latest CEO: `docs/architecture/ceo-update-2026-08-10-wave10.md` (JS climb Option C; soft-assign + tombstones). Honesty priors: `ceo-update-2026-08-09-waves1-2.md`. R-RALPH-2 charter: `ceo-update-2026-08-09-wave3.md`.
...
## Hard-won facts (through 2026-08-11 — Waves 0–2 honesty + Waves 7–10 JS climb)
...
- **JS recovery Option C (Waves 7–10):** ship bar = unlockable/survivor **1.0** (**already met** on Claude Bun dogfood); `oracle_coverage` aspirational; report BOTH (+ `recoverable_oracle_coverage` when tombstones run). Toolkit: `js_recovery_toolkit/` (ensemble → structural/Bun map → defrag/word_map → normalize/semantic/LLM → webcrack/tag-boost → Hungarian soft_assign + unique-token tombstones). Claude dogfood (operator-local `/mnt/c/tmp/reveng_w*`, **never git**): oracle ~29%→33%→52%→55%→**57% (1087/1902)**; recoverable ~**82%** of unique-residue; **874** tombstones; unlockable 1.0. Same-era `.map` materialize = oracle 1.0; Claude SEA `sourcemap_size=0`. PR #149 merged Waves 8–9b; PR #150 Wave 10. Skill: `reveng-js-recovery-climb`.
- **100% ceiling (2026-08-11):** full-oracle 100% on *this* stale-map→SEA pair is **not honestly possible** for no-residue tombstones (algo cannot invent deleted modules). Full-oracle 100% **is** possible with matching `.map` or Bun-embedded SerializedSourceMap. ~18% unique-residue leftover may climb further — not FP-free guaranteed. Pretty close ≠ 100%.
...
- JS recovery full-oracle 100% on SEA — **hard honesty ceiling** at unique-token tombstones (not a temporary algo miss); climb ~18% unique-residue via LLM/graph/Sinkhorn; map/embed path for true oracle 1.0
