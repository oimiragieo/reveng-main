...
class EnhancedTestRunner:
    ...
    def run_performance_benchmarks(self) -> TestSuiteResult:
        ...
        self.test_results.append(suite_result)
        logger.info(
            f"Performance benchmarks completed - {suite_result.success_rate:.1f}% success rate"
        )
    ...
    def run_validation_checks(self) -> List[ValidationResult]:
        ...
        try:
            ...
            assert hasattr(features, "enable_enhanced_analysis")
            assert hasattr(features, "is_any_enhanced_enabled")
        ...
        logger.info(
            f"Validation checks completed - {passed_validations}/{len(validation_results)} passed"
        )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if success_rate < 80:
            recommendations.append(
                "Overall test success rate is below 80%. Review failed tests and address issues."
            )
        ...
        for suite in self.test_results:
            if suite.suite_name == "performance_benchmarks":
                ...
                if failed_perf_tests:
                    recommendations.append(
                        "Performance benchmarks failed. Consider optimizing slow components."
                    )
        ...
        if failed_validations:
            recommendations.append(
                "System validation checks failed. Review configuration and dependencies."
            )
        ...
        for validation in self.validation_results:
            if validation.validation_name == "module_imports" and validation.status == "failed":
                recommendations.append(
                    "Required modules are missing. Run deployment script to install dependencies."
                )

        if not recommendations:
            recommendations.append(
                "All tests and validations passed successfully. System is ready for production use."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced Analysis Test Runner")
    parser.add_argument("--unit-only", action="store_true", help="Run unit tests only")
    parser.add_argument(
        "--integration-only", action="store_true", help="Run integration tests only"
    )
    parser.add_argument(
        "--performance-only", action="store_true", help="Run performance benchmarks only"
    )
    parser.add_argument("--validation-only", action="store_true", help="Run validation checks only")
    parser.add_argument(
    ...
    try:
        ...
        if args.report_file:
            ...
            args.unit_only or args.integration_only or args.performance_only or args.validation_only
        ):
