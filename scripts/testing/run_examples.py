...
class ExamplesRunner:
            ...
            if result.returncode == 0:
                ...
                if sample_path.exists():
                    ...
                    if result.returncode == 0:
                        ...
                        return {
                            "status": "partial",
                            "message": "Template works but sample analysis failed",
                        }
                else:
    ...
    def run_all_examples(self) -> Dict[str, Any]:
        ...
        if advanced_results["status"] == "completed":
            advanced_success = sum(
                1 for r in advanced_results["results"].values() if r == "success"
            )
            total_examples += len(advanced_results["results"])
            successful_examples += advanced_success
            print(
                f"Advanced examples: {advanced_success}/{len(advanced_results['results'])} successful"
            )
